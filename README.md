...
| `<room>_trv` | The actual TRV device the proxy controls. This should be named within Zigbee2MQTT and have its ID synced from there to Home Assistant. Whilst on that inferface, it's a good idea to tell each TRV to use the external sensor if it is already setup and connected to Home Assistant - you should see the 'Local temperature' value change when you have the right sensor option selected. |
| `input_boolean.heating_<room>` | The 'flag' for each room which declares if the TRV is calling for heat (ON) or not (OFF). These aren't directly controlling the relays and instead feed the `input_boolean`s for the floor the room being controlled is on.
| `input_boolean.heating_<zone>` | The combination 'flag' to tell the boiler relay for that zone to turn on if _something_ is calling for heat or turn off if nothing is calling for heat - this essentially acts as an OR logic gate for the TRVs on that floor and avoids resource contention between rooms, potentially breaking boiler with constant switch flickering! |

### Some other useful tidbits
- If you're definitely using Zigbee2MQTT as part of your setup, it's best to leave most of the `topic` MQTT publishes to alone other than the TRV ID (and potentially the end part of the thing you're setting differs to `external_temperature_input`; looking at the 'State' tab for each device should give you an idea of what might be the correct key to publish to.
